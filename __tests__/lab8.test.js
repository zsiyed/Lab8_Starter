describe('Basic user flow for Website', () => {
  ...
  it('Clicking the "Add to Cart" button should change button text', async () => {
    ...
    // Once you have the innerText property, use innerText.jsonValue() to get the text value of it
      ...
      const shadowRoot = await productItems.getProperty('shadowRoot');
      const button = await shadowRoot.$("button");
      await button.click();
      const innerText = await button.getProperty('innerText');
      const val =await innerText.jsonValue(); 
  ...
  it('Checking number of items in cart on screen after reload', async () => {
    ...
    for (var i = 1; i< productItems.length; i++){
      ...
      const val = await innerTxt.jsonValue();
      expect(val).toBe("20");
  }, 10000);
  ...
  it('Checking number of items in cart on screen after removing from cart', async () => {
    ...
    const cartCountNxt = await page.$('#cart-count')
    const innerTextNxt = await cartCountNxt.getProperty('innerText');
    const val = await innerTextNxt.jsonValue();
    expect(val).toBe("0");
  }, 10000);
